...
contract Lottery{
    ...
    function recieve () payable external{
        ...
        players.push(payable(msg.sender)); //add the address of the account that sends
    ...
    function getPlayer(uint i) public view returns(address){
        require(i<players.length);
        return players[i];
    }
 
    function depositEther() external payable {
         require(msg.value >= 0.001 ether);  // will work because you can receive
